...
switch ((int) $delivery_option) {
    case (int) Configuration::get('DPDFRANCE_RELAIS_CARRIER_ID'):
        /* If DPD Relais carrier is selected */
        ...
        if (!empty(Context::getContext()->cookie->dpdfrance_relais_cookie)&&!empty($relay_id)) {
            ...
            /* If error while writing, display error message */
            if (!Db::getInstance()->Execute($sql)) {
                if (version_compare(_PS_VERSION_, '1.5', '<')) {
                    if ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                        Tools::redirect('order.php?step=2&cgv=1&dpdrelais=error');
                    } else {
                        Tools::redirect('order-opc.php?cgv=1&dpdrelais=error#opc_delivery_methods');
                    }
                } elseif ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                    Tools::redirect('index.php?controller=order&step=2&cgv=1&dpdrelais=error');
                } else {
                    Tools::redirect('index.php?controller=order-opc&isPaymentStep=true&cgv=1&dpdrelais=error#carrier_area');
                }
            } else {
                /* It it's all right, redirect to payment step */
                if (version_compare(_PS_VERSION_, '1.5', '<')) {
                    if ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                        Tools::redirect('order.php?step=3&cgv=1&dpdrelais=ok');
                    } else {
                        Tools::redirect('order-opc.php?cgv=1&dpdrelais=ok#opc_delivery_methods');
                    }
                } elseif ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                    Tools::redirect('index.php?controller=order&step=3&cgv=1&dpdrelais=ok');
                } else {
                    Tools::redirect('index.php?controller=order-opc&isPaymentStep=true&cgv=1&dpdrelais=ok#carrier_area');
                }
            }
        } else {
            /* If cookie data is empty, display error message */
            if (version_compare(_PS_VERSION_, '1.5', '<')) {
                if ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                    Tools::redirect('order.php?step=2&cgv=1&dpdrelais=error');
                } else {
        ...
        break;
    case (int) Configuration::get('DPDFRANCE_PREDICT_CARRIER_ID'):
        /* If DPD Predict carrier is selected */
        ...
        if (!(bool) preg_match('#^0[6-7]([0-9]{8})$#', $gsm, $res)||(in_array($res[1], $elimine))) {
            ...
            if (version_compare(_PS_VERSION_, '1.5', '<')) {
                if ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                    Tools::redirect('order.php?step=2&cgv=1&dpdpredict=error');
                } else {
                    Tools::redirect('order-opc.php?cgv=1&dpdpredict=error#opc_delivery_methods');
                }
            } elseif ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                Tools::redirect('index.php?controller=order&step=2&cgv=1&dpdpredict=error');
            } else {
                Tools::redirect('index.php?controller=order-opc&isPaymentStep=true&cgv=1&dpdpredict=error#carrier_area');
            }
        } else {
            ...
            if (!Db::getInstance()->Execute($sql)) {
                /* If error while writing in database : display an error message */
                echo '<div class="alert error">Un probl̬me est survenu lors de l\'enregistrement de votre num̩ro de t̩l̩phone, merci de r̩essayer</div>';
                echo '<a href="javascript:history.back()">Retour</a>';
            }
            if (version_compare(_PS_VERSION_, '1.5', '<')) {
                if ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                    Tools::redirect('order.php?step=3&cgv=1&dpdpredict=ok');
                } else {
                    Tools::redirect('order-opc.php?cgv=1&dpdpredict=ok#opc_delivery_methods');
                }
            } elseif ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                Tools::redirect('index.php?controller=order&step=3&cgv=1&dpdpredict=ok');
            } else {
                Tools::redirect('index.php?controller=order-opc&isPaymentStep=true&cgv=1&dpdpredict=ok#carrier_area');
            }
        }
        break;
    default:
        ...
        if (version_compare(_PS_VERSION_, '1.5', '<')) {
            if ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
                Tools::redirect('order.php?step=3&cgv=1&dpdfrance=bypass');
            } else {
                Tools::redirect('order-opc.php?cgv=1&dpdfrance=bypass#opc_delivery_methods');
            }
        } elseif ((int) Configuration::get('PS_ORDER_PROCESS_TYPE')==0) {
            Tools::redirect('index.php?controller=order&step=3&cgv=1&dpdfrance=bypass');
        } else {
            Tools::redirect('index.php?controller=order-opc&isPaymentStep=true&cgv=1&dpdfrance=bypass#carrier_area');
        }
        break;
}
exit;
